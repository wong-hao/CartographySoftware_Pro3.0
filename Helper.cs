...
namespace SMGI_Common
{
    public class Helper
    {
        ...
        /// <returns></returns>
        public static async Task<IGPResult> CreateTempWorkspace(string gdbFilePath, string gdbFileName)
        {
            var fullPath = Path.Combine(gdbFilePath, gdbFileName);
            ...
            {
                // 验证输入的gdbFilePath不为空且以.gdb结尾
                if (string.IsNullOrWhiteSpace(gdbFilePath) || !gdbFileName.EndsWith(".gdb")) return tGeoResult;
            }
            catch (GeodatabaseNotFoundOrOpenedException)
            {
                ...
                tGeoResult = await Geoprocessing.ExecuteToolAsync("management.CreateFileGDB", parameters, null, cts.Token,
                    (eventName, o) => { Debug.WriteLine($@"GP event: {eventName}"); }, executeFlags);

                // 检查创建操作是否成功
                if (tGeoResult.ErrorCode != 0)
                {
                    Geoprocessing.ShowMessageBox(tGeoResult.Messages, "GP Messages",
                        tGeoResult.IsFailed ? GPMessageBoxStyle.Error : GPMessageBoxStyle.Default);
                    MessageBox.Show("CreateFileGDB failed. " + tGeoResult.ErrorMessages);
                }
                else
                // 创建成功，尝试再次打开工作空间
                {
        ...
        public static void InitializeTempWorkspace()
        {
            // 初始化系统临时工作空间
            CreateTempWorkspace(GApplication.Application.AppDataPath, "MyWorkspace.gdb");

            // 初始化项目临时工作空间
            CreateTempWorkspace(GApplication.Application.ProjectHomeFolderPath, "MyWorkspace.gdb");
        }
    }
}
